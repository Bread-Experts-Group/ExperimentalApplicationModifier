...
class MultiBufferSource(around: Any) : MimickedClass(around) {
	companion object {
		val clazz: Class<*> = loadClass(net_minecraft_client_renderer_MultiBufferSource)
		val classDesc: ClassDesc = ClassDesc.of(clazz.name)
		val mimicClassDesc: ClassDesc = ClassDesc.of(MultiBufferSource::class.qualifiedName)
	}

	fun getBuffer(renderType: RenderType): VertexConsumer = VertexConsumer(
